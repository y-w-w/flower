...
<%
	...
	ordersCount = ordersDao.getCount2();//得到会员的总数量值
	String str = null;
	String string = null;
	...
	ArrayList<OrdersStatu> orderList = ordersDao.queryByPageStatus2(currentPage,pageSize);
	...
	/* for(int t=0 ;t<orderList.size();t++){
		if("1".equals(orderList.get(i).getOrdersStatus())){
	str="未发货";
		}
		else if("2".equals(orderList.get(i).getOrdersStatus())){
	str="已发货";
		}
		else if("3".equals(orderList.get(i).getOrdersStatus())||"5".equals(orderList.get(i).getOrdersStatus())){
	str="交易成功";
		}
		else if("0".equals(orderList.get(i).getOrdersStatus())&&"1".equals(orderList.get(i).getOrdersPaymen())){
	str="申请退款";
		}
		else if("0".equals(orderList.get(i).getOrdersStatus())&&"0".equals(orderList.get(i).getOrdersPaymen())){
	str="关闭订单";
		}
	
		else if("4".equals(orderList.get(i).getOrdersStatus())&&"0".equals(orderList.get(i).getOrdersPaymen())){
	str="已经退款";
		} */
	}
...
function ChangeStatus(memberid,status){
		...
		$.post("AdminChangeOrdersStatesServlet",{memberid:memberid,status:status},
			function(rs){
			if(rs>0){
				alert("发货成功");
				if(status=="1"){
									...
									<tr>
										<td width="46%" valign="middle"><table width="100%"
												...
												<tr>
													...
													<td width="95%" class="STYLE1"><span class="STYLE3">你当前的位置</span>：[订单管理]-[显示已发货订单]</td>
												</tr>
											</table></td>
										...
										<tr>
											...
											<td height="20" bgcolor="#FFFFFF"><div align="center">
													<span class="STYLE4" id="memberStatus"><Input
														...
														width="16" height="16" 
													value="<%=list[j]%>"
													onclick="ChangeStatus(${Order.ordersId},${Order.ordersStatus});" />&nbsp;
														&nbsp;</span>
												</div></td>
											<%
												} else {
							...
							<td><table width="100%" border="0" cellspacing="0"
									...
									<tr>
										...
										<td><table border="0" align="right" cellpadding="0"
												...
												<tr>
													<td width="40"><a
														href="ordersManagerstatus2.jsp?page=1"><img
															src="tab/images/first.gif" width="37" height="15" /></a></td>
													<td width="45"><a
														href="ordersManagerstatus2.jsp?page=<%=lastPage%>"><img
															src="tab/images/back.gif" width="43" height="15" /></a></td>
													<td width="45"><a
														href="ordersManagerstatus2.jsp?page=<%=nextPage%>"><img
															src="tab/images/next.gif" width="43" height="15" /></a></td>
													<td width="40"><a
														href="ordersManagerstatus2.jsp?page=<%=pageCount%>"><img
