...
<!-- 收货人信息 -->
  ...
  <form method="post" action="MemberOrder_SendServlet" onsubmit="submitonce(this)">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="dddddd">
          ...
          <c:forEach items="${requestScope.carlist}" var="item" varStatus="status">     
                
                  <tr align="center"> 
                    <td bgcolor="#FFFFFF">${item.CID}</td>
                    <td align="left" bgcolor="#FFFFFF" style="text-align:left;"><a href="MemberDetailServlet?comId=${item.CID}" target="_blank"><img src="../admin/upload/thumb/${item.url }" alt="${item.CNAME }" width="48" height="51" align="absmiddle" /><u>${item.CNAME }</u></a> </td>
                    <td bgcolor="#FFFFFF"> ${item.CPP }
                    </td>
                    <td bgcolor="#FFFFFF">${item.SCN }</td>
                    <td bgcolor="#FFFFFF">${item.getTotalprice()}元 </td> 
                  </tr>
         </c:forEach>          
